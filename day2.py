...
with open('input.txt') as f:
    for line in f:
        a, _, b = line.strip()
        # adding points for my shape
        score += shape_points[b]
        # adding points for the result
        # 1st option: draw
        if elf_points[a] == shape_points[b]:
            score += 3
        # 2nd option: win
        elif ((a == 'A') & (b == 'Y')) | ((a == 'B') & (b == 'Z')) | ((a == 'C') & (b == 'X')):
            score += 6
        # 3rd option: lose
        else:
            pass
...
# e.g. if elf shows rock and I need to lose therefore I need to show scissors and get 2 points
lose_opposit_points = {'A': 3, 'B': 1, 'C': 2}
# e.g. if elf shows rock and I need to win therefore I need to show paper and get 3 points
win_opposit_points = {'A': 2, 'B': 3, 'C': 1}
...
with open('input.txt') as f:
    for line in f:
        ...
        # you need to lose
        if b == 'X':
            score += lose_opposit_points[a] + 0
        # you need to draw
        elif b == 'Y':
            score += elf_points[a] + 3
        elif b == 'Z':
            score += win_opposit_points[a] + 6

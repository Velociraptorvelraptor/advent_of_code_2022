...
with open('input.txt') as f:
    for n, pair in enumerate(f):
        a, b = pair.split(',')
        a_1, a_2 = a.split('-')
        b_1, b_2 = b.split('-')
        a_set = set(range(int(a_1), int(a_2)+1))
        b_set = set(range(int(b_1), int(b_2)+1))
        if a_set.issuperset(b_set) or b_set.issuperset(a_set):
            counter_superset += 1
        if not (a_set.isdisjoint(b_set) and b_set.isdisjoint(a_set)):
            counter_subset += 1

print(f'Number of totally overlapped sets is: {counter_superset}.')
print(f'Number of overlapping sets is: {counter_subset}.')
